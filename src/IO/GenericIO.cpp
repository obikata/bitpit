...
/*!
 * Copies entire file into a char array. BEWARE: only valid characters will be copied.
  F.E. ON Windows OS, read-from-file \r\n characters take 2 position in the stream, but are accounted
  as a unique character (endline) after c++ translation with read function.
 * @param[in] str file stream to be copied; file needs already to be opened
...
void copyUntilEOFInString(std::fstream &str, char *&buffer, int &length)
{
    ...
    //This number can be less than length, especially while reading OS-Windows files.
    //See method docs
