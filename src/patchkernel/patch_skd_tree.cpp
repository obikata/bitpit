                                          ...
                                          double maxDistance, long *ids, int *ranks, double *distances) const
{
    ...
    if (getPatch().isPartitioned()){
        ...
        // Instantiate global container for distances, ids and ranks
        std::vector<std::pair<double,int>> globalDistancesRanks(nGlobalPoints, std::pair<double, int>(std::numeric_limits<double>::max(), m_rank));
        std::vector<long> globalIds(nGlobalPoints, Cell::NULL_ID);
        ...
        std::vector<SkdCellInfo> dri_data(nGlobalPoints);
        for (std::size_t ip = 0; ip < nGlobalPoints; ip++){
