...
void DiscreteStencil<weight_t>::optimize(double tolerance)
{
    ...
    for (std::size_t n = 0; n < nItems; ++n) {
        if (isWeightNeglibile(n, tolerance)) {
...
bool DiscreteStencil<weight_t>::isWeightNeglibile(std::size_t pos, double tolerance)
{
    return (std::abs(m_weights[pos]) <= tolerance);
}
...
bool DiscreteStencil<weight_t>::isWeightNeglibile(std::size_t pos, double tolerance)
{
    return (norm2(m_weights[pos]) <= tolerance);
}
...
{
    const std::size_t other_nItems = size();
