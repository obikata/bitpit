 ...
 */
template<typename stencil_t>
long DiscretizationStencilSolverAssembler<stencil_t>::getRowNZCount(long row) const
{
    const stencil_t &stencil = (*m_stencils)[row];
 ...
 */
template<typename stencil_t>
void DiscretizationStencilSolverAssembler<stencil_t>::getRowPattern(long row, ConstProxyVector<long> *pattern) const
{
 ...
 */
template<typename stencil_t>
void DiscretizationStencilSolverAssembler<stencil_t>::getRowValues(long row, ConstProxyVector<double> *values) const
{
    _getRowValues(row, values);
}
...
void DiscretizationStencilSolverAssembler<stencil_t>::_getRowValues(long row, ConstProxyVector<double> *values) const
{
    // Get stencil information
    const stencil_t &stencil = (*m_stencils)[row];
 ...
 */
template<typename stencil_t>
double DiscretizationStencilSolverAssembler<stencil_t>::getRowConstant(long row) const
{
    return _getRowConstant(row);
}
...
template<typename stencil_t>
template<typename U, typename std::enable_if<std::is_fundamental<U>::value>::type *>
double DiscretizationStencilSolverAssembler<stencil_t>::_getRowConstant(long row) const
{
    // Get stencil information
    const stencil_t &stencil = (*m_stencils)[row];
...
void DiscretizationStencilSolver<stencil_t>::update(std::size_t nRows, const long *rows, const std::vector<stencil_t> &stencils)
{
    ...
    for (std::size_t n = 0; n < nRows; ++n) {
        ...
        m_constants[row] = assembler.getRowConstant(row);
    }
}
